...
void c_keys::UpdateKeys()
{
	...
	DWORD bytesRead = 0;
	VMMDLL_MemReadEx(mem.vHandle, this->win_logon_pid | VMMDLL_PID_PROCESS_WITH_KERNELMEMORY, gafAsyncKeyStateExport, reinterpret_cast<PBYTE>(&state_bitmap), 64, &bytesRead, VMMDLL_FLAG_NOCACHE);
	if (bytesRead != 64) {
		LOG("[Keyboard] DMA read failed! bytesRead=%d, addr=0x%llx, pid=%d\n", bytesRead, gafAsyncKeyStateExport, this->win_logon_pid);
		memset(state_bitmap, 0, 64);
		return;
	}
...
bool c_keys::IsKeyDown(uint32_t virtual_key_code)
{
	...
	if (std::chrono::system_clock::now() - start > std::chrono::milliseconds(10))
	{
		...
		// Debug: 检查是否有大量按键同时处于按下状态（说明读到脏数据）
		static int debugCounter = 0;
		if (++debugCounter % 50 == 0) { // 每5秒打印一次
			int pressedCount = 0;
			for (int vk = 0; vk < 256; ++vk) {
			...
			if (pressedCount > 5) {
				LOG("[Keyboard] WARNING: %d keys appear pressed simultaneously - likely bad data!\n", pressedCount);
			}
		}
	}
